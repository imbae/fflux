...
public sealed class VideoDecoder : IVideoDecoder
{
    ...
    // 마지막으로 읽은 비디오 패킷의 크기 (bytes). VideoFrame.PacketSizeBytes에 전달됩니다.
    private int _lastVideoPacketSize;
    ...
    private unsafe ReadPacketResult ReadNextPacket()
    {
        ...
        {
            _lastVideoPacketSize = packet->size;
            return ReadPacketResult.VideoPacket;
        }

        return ReadPacketResult.NonVideoPacket;
    }
    ...
    private unsafe VideoFrame? ReceiveNextFrame()
    {
        ...
        var videoFrame = _converter!.Convert(
            convertSrc, convertSrc->width, convertSrc->height, ts, _lastVideoPacketSize);
