...
public sealed class VideoFrame : IDisposable
{
    ...
    /// <summary>
    /// 이 프레임이 디코딩된 원본 AVPacket의 크기 (bytes).
    /// 실시간 비트레이트 계산에 사용합니다. 0이면 패킷 크기 미제공.
    /// </summary>
    public int PacketSizeBytes { get; }

    /// <param name="isPooled">true이면 Dispose() 시 Data를 ArrayPool에 반환합니다.</param>
    /// <param name="packetSizeBytes">원본 패킷 크기 (bytes). 비트레이트 계산용.</param>
    public VideoFrame(int width, int height, int stride, byte[] data, TimeSpan timestamp,
                      bool isPooled = false, int packetSizeBytes = 0)
    {
        Width           = width;
        Height          = height;
        Stride          = stride;
