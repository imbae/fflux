...
public partial class PlayerPage : Page
{
    ...
    // 레이아웃 상수 — XAML ControlBar 점유 높이와 맞춥니다.
    private const double ControlBarHeight = 80;   // 트랜스포트 바 실제 점유 높이 (슬라이더 + 버튼 행)

    /// <summary>
    /// RootGrid와 MisbMetadataPanel의 MaxWidth/MaxHeight를 뷰포트 크기로 제한합니다.
    /// <para>
    /// 단일 RootGrid 구조이므로 페이지 전체 높이 = RootGrid 유효 높이입니다.
    ...
    private void ConstrainLayout(double width, double height)
    {
        ...
        // 패널 MaxHeight = 전체 높이 - 하단 컨트롤 바
        if (height > 0) MisbPanel.MaxHeight = height - ControlBarHeight;
    }
