...
/// </summary>
// IMediaPositionProvider는 fflux.AiSubtitle에 정의됨.
...
public sealed partial class PlayerViewModel : ObservableObject, IDisposable
{
    ...
    private CancellationTokenSource? _previewCts;   // 미리보기 프레임 렌더 취소용
    ...
    private IStreamCopyRecorder? _recorder;
    // 현재 재생/녹화 중인 소스 경로 (라이브 URL 포함)
    ...
    // ── 실시간 재생 통계 (RenderFrame — UI 스레드 전용) ────────────────
    //
    // 1초 윈도우로 FPS / 비트레이트를 측정합니다.
    ...
    // ── 라이선스 ─────────────────────────────────────────────────────
    // MainWindowViewModel.IsPremiumUser가 전역 진실의 원천입니다.
    // PlayerPage.xaml 의 {Binding IsPremiumUser} 은 이 프로퍼티를 통해 포워딩됩니다.

    /// <summary>
    /// Pro 구독 여부 — <see cref="MainWindowViewModel.IsPremiumUser"/> 에서 포워딩.
    /// </summary>
    public bool IsPremiumUser => _mainVm.IsPremiumUser;
    ...
    /// <summary>우측 미디어 정보 패널 표시 여부.</summary>
    [ObservableProperty]
    private bool _isMediaInfoVisible;

    /// <summary>현재 열린 파일의 미디어 정보. 파일 미열기 또는 로드 실패 시 null.</summary>
    [ObservableProperty]
    ...
    /// <summary>현재 측정된 렌더링 FPS (1초 윈도우 평균).</summary>
    [ObservableProperty]
    private double _currentFps;

    /// <summary>현재 비트레이트 (Kbps, 1초 윈도우 평균).</summary>
    [ObservableProperty]
    private double _currentBitrateKbps;

    /// <summary>현재 프레임 번호 (PTS × FrameRate 추정). 파일 시작 기준.</summary>
    [ObservableProperty]
    ...
    /// <summary>녹화 경과 시간 텍스트 (예: "● 녹화 중  00:42").</summary>
    [ObservableProperty]
        ...
        ILogger<PlayerViewModel> logger)
    {
        ...
        {
            if (e.PropertyName == nameof(MainWindowViewModel.IsPremiumUser))
    ...
    // 서브모듈이 있으면 PlayerViewModel.Misb.cs / PlayerViewModel.AiSubtitle.cs
    // 의 partial 에서 실제 구현이 제공됩니다.
    // XAML 바인딩은 서브모듈 없이도 조용히 비활성화됩니다.

#if !MISB
    [RelayCommand] private Task ToggleMisbAsync()    => Task.CompletedTask;
    [RelayCommand] private void ToggleMisbPanel()    { }
#endif

#if !AI_SUBTITLE
    [RelayCommand] private void ToggleRealTimeTranslation() { }
#endif
    ...
    /// <summary>우측 미디어 정보 패널 표시/숨김 토글.</summary>
    [RelayCommand]
    ...
    /// <summary>
    /// 녹화 시작/중지 토글.
    /// 녹화 중이면 <see cref="IStreamCopyRecorder.StopAsync"/>를 호출하고,
    /// 아니면 현재 소스를 Stream Copy로 녹화 시작합니다.
    /// </summary>
    [RelayCommand]
    private async Task ToggleRecordingAsync()
    {
        ...
        if (!IsFileOpen || _currentSourcePath is null)
        {
            _logger.LogWarning("녹화 시작 실패: 파일이 열려 있지 않습니다.");
            return;
        }
        ...
        try
        {
            _recorder = _services.GetRequiredService<IStreamCopyRecorder>();
            _recorder.DurationUpdated += OnRecordingDurationUpdated;

            var startTime = IsLiveStream ? TimeSpan.Zero : TimeSpan.FromSeconds(PositionSeconds);
            await _recorder.StartAsync(_currentSourcePath, dlg.FileName, startTime);

            IsRecording = true;
            RecordingDurationText = "● 녹화 중  00:00";
            _logger.LogInformation("녹화 시작: {Path}", dlg.FileName);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "녹화 시작 실패");
            StatusText = $"녹화 시작 실패: {ex.Message}";
            CleanupRecorder();
        }
    }

    /// <summary>GIF 구간 내보내기.</summary>
    [RelayCommand]
    private async Task ExportGifAsync()
    {
        if (!IsFileOpen || _currentSourcePath is null || IsLiveStream)
        {
            _logger.LogWarning("GIF 내보내기: 파일이 열려 있지 않거나 라이브 스트림입니다.");
            return;
        }
        ...
        panel.Children.Add(new System.Windows.Controls.TextBlock
        {
            Text = "※ GIF는 최대 폭 480px, 10fps로 자동 최적화됩니다.",
            FontSize = 11,
        ...
        if (!double.TryParse(durationBox.Text.Trim(), out var durSec) || durSec <= 0)
        {
            StatusText = "GIF 내보내기: 잘못된 구간 길이입니다.";
            return;
        }
        ...
        _logger.LogInformation("GIF 내보내기: {Start:g} + {Dur:g}", startTime, gifDuration);

        try
        {
            var exporter = _services.GetRequiredService<IGifExporter>();
            var progress = new Progress<double>(p =>
                Application.Current.Dispatcher.InvokeAsync(
                    () => StatusText = $"GIF 내보내기 {p:P0}"));

            await exporter.ExportAsync(
                _currentSourcePath, dlg.FileName,
                startTime, gifDuration,
                maxWidth: 480, targetFps: 10.0,
                progress: progress);

            StatusText = $"GIF 저장 완료: {Path.GetFileName(dlg.FileName)}";
            _logger.LogInformation("GIF 저장 완료: {Path}", dlg.FileName);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "GIF 내보내기 실패");
            StatusText = $"GIF 내보내기 실패: {ex.Message}";
        }
    }
    ...
    private async Task StopRecordingAsync()
    {
        ...
        try
        {
            ...
            _logger.LogInformation("녹화 완료: {Duration}", RecordingDurationText);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "녹화 중지 중 오류");
        }
        finally
        {
            CleanupRecorder();
    ...
    private void OnRecordingDurationUpdated(object? sender, TimeSpan elapsed)
    {
        // 레코더가 예상치 않게 중단된 경우 (오류 등) UI 상태 정리
        if (_recorder is { IsRecording: false })
        {
            var errorMsg = _recorder.LastError;
            Application.Current.Dispatcher.InvokeAsync(() =>
            {
                CleanupRecorder();
                if (errorMsg is not null)
                    StatusText = $"녹화 실패: {errorMsg}";
            });
            return;
        }

        var text = $"● 녹화 중  {FormatTs(elapsed)}";
        Application.Current.Dispatcher.InvokeAsync(
...
// ── 자막 커맨드 ──────────────────────────────────────────────────
    ...
    // 재생 위치에 해당하는 자막 텍스트와 색상을 업데이트합니다.
    private void UpdateSubtitle(TimeSpan position)
    {
        ...
        var entry = FindSubtitleEntry(position);
        SubtitleText = entry?.Text ?? string.Empty;

        // SRT <font color="..."> 태그에서 추출된 색상 적용
        if (entry?.Color is { Length: > 0 } hex)
        {
            try
            {
                var color = (System.Windows.Media.Color)ColorConverter.ConvertFromString(hex);
                SubtitleColor = new System.Windows.Media.SolidColorBrush(color);
            }
            catch
            {
                SubtitleColor = Brushes.White;
    ...
    /// <summary>
    /// <paramref name="position"/>으로 절대 시크합니다.
    /// <see cref="SeekRelativeAsync"/>와 달리 <see cref="PositionSeconds"/>를 두 번 읽지 않아
    /// 재생 중 클릭 시 목표 위치가 바뀌는 경쟁 조건이 없습니다.
    /// </summary>
    public async Task SeekToAsync(TimeSpan position)
    {
        if (!IsFileOpen || IsLiveStream || DurationSeconds == 0) return;
        var clamped = TimeSpan.FromSeconds(Math.Clamp(position.TotalSeconds, 0, DurationSeconds));
        _isUpdatingPositionFromPlayback = true;
        PositionSeconds = clamped.TotalSeconds;
        _isUpdatingPositionFromPlayback = false;
        await UserSeekAsync(clamped);
    }
    ...
    /// <summary>
    /// 파일을 열되 재생은 시작하지 않습니다.
    /// 자막 편집기 등 외부 모듈에서 비디오를 미리보기 용도로 로드할 때 사용합니다.
    /// </summary>
    public Task OpenFileWithoutPlayAsync(string filePath) => OpenSourceInternalAsync(filePath, isLive: false, autoPlay: false);
    ...
    private async Task OpenSourceInternalAsync(string source, bool isLive, bool autoPlay = true)
    {
        StopPlaybackLoop(pauseAudio: false);
        _pausedPosition = TimeSpan.Zero;
        ResetPlaybackStats();
        ...
        // ── 미디어 정보 로드 (파일 전용, 백그라운드 — 재생에 영향 없음) ──
        CurrentMediaInfo = null;
        if (!isLive)
            _ = LoadMediaInfoAsync(source);

        if (autoPlay)
        {
            // UpdateStatus는 StartPlaybackLoop 내부에서 처리됩니다.
            StartPlaybackLoop();
        }
        else
        {
    ...
    private async Task UserSeekAsync(TimeSpan position)
    {
        ...
        {
            // 정지/일시정지 상태에서도 시크 위치의 프레임을 미리보기로 렌더링합니다.
            // 이전 미리보기 요청을 취소하고, 재생 루프가 완전히 종료된 후 실행합니다.
            _previewCts?.Cancel();
            _previewCts?.Dispose();
            _previewCts = new CancellationTokenSource();

            // 재생 루프 태스크가 실제로 종료할 때까지 대기 (최대 300ms).
            // StopPlaybackLoop은 CTS만 취소하고 태스크를 await하지 않으므로
            // 루프가 아직 _videoDecoder에서 DecodeAsync 중일 수 있습니다.
            var pendingLoops = new[] { _videoLoopTask, _audioLoopTask }
                .Where(t => t is { IsCompleted: false })
                .Cast<Task>()
                .ToArray();
            if (pendingLoops.Length > 0)
            {
                try { await Task.WhenAll(pendingLoops).WaitAsync(TimeSpan.FromMilliseconds(300)); }
                catch { /* OperationCanceledException 또는 타임아웃 — 무시 */ }
            }

            _ = RenderPreviewFrameAsync(position, _previewCts.Token);
        }
    }

    /// <summary>
    /// 재생하지 않고 <paramref name="position"/>의 비디오 프레임 한 장을 디코드하여 화면에 표시합니다.
    /// <para>
    /// 호출 전 재생 루프가 완전히 종료되어야 합니다 (<see cref="UserSeekAsync"/> 참조).
    /// <paramref name="ct"/>가 취소되면 렌더링을 중단합니다.
    /// </para>
    /// </summary>
    private async Task RenderPreviewFrameAsync(TimeSpan position, CancellationToken ct)
    {
        if (_videoDecoder == null || !_videoDecoder.IsOpen) return;
        try
        {
            var frame = await _videoDecoder.SeekAndDecodeAtAsync(position);
            if (frame is null || ct.IsCancellationRequested) { frame?.Dispose(); return; }

            await Application.Current.Dispatcher.InvokeAsync(() =>
            {
                if (!ct.IsCancellationRequested) RenderFrame(frame);
                frame.Dispose();
            });
        }
        catch
        {
            // 미리보기 렌더링 실패는 재생에 영향 없음 — 무시
        }
    }
    ...
    private void RenderFrame(VideoFrame frame)
    {
        ...
        // 실시간 통계 업데이트 (FPS / 비트레이트 / 프레임 번호)
        UpdatePlaybackStats(frame);
    ...
    /// <summary>
    /// 백그라운드에서 미디어 정보를 읽어 <see cref="CurrentMediaInfo"/>를 갱신합니다.
    /// 실패해도 플레이어 재생에 영향 없이 로그만 남깁니다.
    /// </summary>
    private async Task LoadMediaInfoAsync(string filePath)
    {
        ...
        {
            _logger.LogWarning(ex, "미디어 정보 로드 실패: {File}", filePath);
        }
    }
    ...
    /// <summary>
    /// RenderFrame()마다 호출: 1초 윈도우로 FPS와 비트레이트를 측정하고
    /// PTS × FrameRate로 프레임 번호를 추정합니다.
    /// UI 스레드(RenderFrame)에서만 호출됩니다.
    /// </summary>
    private void UpdatePlaybackStats(VideoFrame frame)
    {
        _statsFpsCount++;
        _statsBitrateBytes += frame.PacketSizeBytes;
        ...
        // 프레임 번호: PTS 기반 추정 (프레임레이트 × 초)
        if (_frameRate > 0)
            CurrentFrameNumber = (long)Math.Round(frame.Timestamp.TotalSeconds * _frameRate);
    }
    ...
    public void Dispose()
    {
...
#if AI_SUBTITLE
        // 0. 실시간 번역 정리
        DisposeAiSubtitleResources();
#endif
#if MISB
        // 1. MISB 리소스 정리
        DisposeMisbResources();
#endif
        ...
        if (_recorder is not null)
        {
            _recorder.StopAsync().GetAwaiter().GetResult();
            CleanupRecorder();
        }
